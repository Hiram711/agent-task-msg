...
状态：**历史研究提案；已停止推进核心替换及启动器方案。补丁未编译、未安装，也未完成修改后的运行验收。**
本目录不被技能安装器作为运行文件复制，不代表技能已经修好桌面权限通知。

当前采用技能内的简化流程：自动审查模式不预提醒；明确人工审批模式时，在 `request_permissions` 前主动通知。
被自动审查拒绝且确实需要用户决定时复用提问通知。详见 [当前流程](../references/codex.md#权限申请前按审批模式过滤)。
它不宣称修复了原生 hook，也不要求用户修改 Codex 启动方式。
...
本提案仅保留定位证据，不属于当前安装方案，不声称已让本机桌面这一路生效。
